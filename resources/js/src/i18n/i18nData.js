// en, de, fr, pt
export default {
    en: {
        meta: {
            title: {
                home: 'League of Legends french community',
                probuilds: 'League of Legends pro players builds and spectate them',
                community: 'Featured twitch channels, builds and players',
            },
            description: {
                home: 'Evil Spartan is a League of Legends french community. Featured twitch channels, builds and players!',
                probuilds: "Evil Spartan is a quick guides tool for League of Legends players. Its primary function is to provide the League of Legends community easy access to Professional Player's builds across all regions",
                community: 'Evil Spartan is a League of Legends french community, Featured twitch channels, builds and players!',
            }
        },
        404: {
            backToHome: "Back to Home"
        },
        nav: {
            home: "Home",
            probuilds: "Probuilds",
            spectate: "Spectate",
            community: "Community"
        },
        home: {
            welcome: 'Welcome to Probuilds',
            welcomeText: "League of Legends professional player's <strong class='text-gold'>builds</strong> across all regions",
            welcomeBrand: "Welcome to Evil Spartan",
            welcomeTextBrand: "League of Legends <strong class='text-gold'>french community</strong>"
        },
        LiveFeed: {
            title: 'Live Feed',
            player: "Player",
            level: 'Level',
            vs: "VS.",
            kda: 'KDA',
            gold: 'Gold',
            keystone: 'Keystone',
            summoners: 'Summoner Spells',
            build: 'Build',
            all: 'All',
            allRegion: 'All Regions',
            region: 'Region',
            probuilds: "Probuilds",
            probuildsTitle: "Best players builds"
        },
        Match: {
            winningTeam: "Winning Team",
            losingTeam: "Losing Team",
            bans: "Bans",
            titleIfNull: "None",
            duration: "Game duration"
        },
        Item: {
            buildOrder: "Build Order",
            timeline: "Timeline"
        },
        Summoner: {
            player: "Player",
            league: "League",
            leaguePoints: "Points",
            items: "Items",
            finalItems: "Final Build",
            summoners: "Summoner Spells"
        },
        Champion: {
            name: 'Champion',
            description: 'Description'
        },
        SearchBanner: {
            loading: 'Loading...',
            searchInput: 'Search champion...'
        },
        Spell: {
            abilities: "Abilities",
            abilitiesOrder: "Abilities Order",
            1: "Q",
            2: "W",
            3: "E",
            4: "R"
        },
        Rune: {
            runes: "Reforged Runes"
        },
        Global: {
            sold: "Sold",
            Regions: "Regions",
            understood: "Understood",
            next: "Next",
            previous: "Previous",
            skip: "Skip"
        },
        Special: {
            AttackSpeed: "Attack Speed",
            Adaptive: "Adaptive",
            Armor: "Armore",
            MagicRes: "Magic Resistance",
            HealthScaling: "Health Scaling",
            CDRScaling: "Cooldown Reduction"
        },
        Message: {
            noData: "No Data",
            noLivefeedData: "Livefeed is not available yet for the requested criteria.",
            noMatchesData: "No match available for the requested region.",
            loginWaitTitle: "Please wait",
            loginWaitMessage: "We are login you to the plateform...",
            loginTitle: "Successful logged in",
            loginMessage: "You are logged in to the platform!",
            logoutTitle: "Successful logged out",
            logoutMessage: "You are logged out to the platform, See you soon!",
            notAvailable: "Soon Available",
            notAvailableDescription: "We are working on it! You can already login to get your account on <strong>Evil Spartan</strong>",
            tourLoginMessage: "You can already login to get your account on <strong>Evil Spartan</strong",
        },
        Spectate: {
            watch: "Watch",
            OS: "Your OS",
            restartDownload1: "Your download will begin in a moment.",
            restartDownload2: "If it doesn't,",
            restartDownload3: "restart the download",
            macMessage: "After running <strong class='text-primary'>[Terminal]</strong>, copy and paste the message below.",
        },
        Community: {
            title: "Evil Spartan Community",
            description: "Featured twitch channels, builds and players",
        }
    },
    de: {},
    fr: {
        meta: {
            title: {
                probuilds: 'Builds et game en live des joueurs pro League of Legends'
            },
            description: {
                probuilds: ''
            }
        },
        404: {
            backToHome: "Revenir à l'accueil"
        },
        nav: {
            home: "Home",
            probuilds: "Probuilds",
            spectate: "Regarder des parties",
            community: "Communauté"
        },
        home: {
            welcome: 'Bienvenue sur Probuilds',
            welcomeText: "<strong class='text-gold'>Builds</strong> des joueurs professionnels sur League of Legends dans toutes les régions",
            welcomeBrand: "Bienvenue sur Evil Spartan",
            welcomeTextBrand: "<strong class='text-gold'>Communauté française</strong> sur League of Legends"
        },
        LiveFeed: {
            title: "Flux en direct",
            player: "Joueur",
            level: 'Niveau',
            vs: "VS.",
            kda: 'KDA',
            gold: 'Gold',
            keystone: 'Keystone',
            summoners: "Sorts d'invocateur",
            build: 'Build',
            all: 'Tout',
            allRegion: 'Toutes les régions',
            region: 'Region',
            probuilds: "Probuilds",
            probuildsTitle: "Builds des meilleurs joueurs"
        },
        Match: {
            winningTeam: "Équipe Gagnante",
            losingTeam: "Équipe Perdante",
            bans: "Bans",
            titleIfNull: "Aucun",
            player: "Joueur",
            league: "Ligue",
            leaguePoints: "Points",
            duration: "Durée de la partie"
        },
        Item: {
            buildOrder: "Ordre d'achat",
            timeline: "Chronologie"
        },
        Summoner: {
            player: "Joueur",
            league: "Ligue",
            leaguePoints: "Points",
            items: "Items",
            finalItems: "Build final",
            summoners: "Sorts d'invocateur",
        },
        Champion: {
            name: 'Champion',
            description: 'Description'
        },
        SearchBanner: {
            loading: 'Chargement...',
            searchInput: 'Rechercher un champion...'
        },
        Spell: {
            abilities: "Compétences",
            abilitiesOrder: "Ordre des compétences",
            1: "A",
            2: "Z",
            3: "E",
            4: "R"
        },
        Rune: {
            runes: "Runes reforgées"
        },
        Global: {
            sold: "Vendu",
            Regions: "Regions",
            next: "Suivant",
            previous: "Précédent",
            skip: "Passer",
            understood: "Compris"
        },
        Special: {
            AttackSpeed: "Vitesse d'attaque",
            Adaptive: "Adaptif",
            Armor: "Armure",
            MagicRes: "Résistance magique",
            HealthScaling: "Points de Vie",
            CDRScaling: "Réduction des délais"
        },
        Message: {
            noData: "Aucune donnée",
            noLivefeedData: "Le livefeed n'est pas encore disponible pour les critères demandés.",
            noMatchesData: "Aucun match disponible pour la région demandée.",
            loginWaitTitle: "Veuillez patienter",
            loginWaitMessage: "Nous vous connectons à la plateforme...",
            loginTitle: "Connexion réussie",
            loginMessage: "Vous êtes maintenant connecté à la plateforme !",
            logoutTitle: "Déconnexion réussie",
            logoutMessage: "Vous êtes maintenant déconnecté de la plateforme, à bientôt !",
            notAvailable: "Bientôt disponible",
            notAvailableDescription: "Nous travaillons sur cette page! Vous pouvez déjà vous connecter pour obtenir votre compte sur <strong>Evil Spartan</strong>",
            tourLoginMessage: "Vous pouvez déjà vous connecter pour obtenir votre compte sur <strong>Evil Spartan</strong>",
        },
        Spectate: {
            watch: "Regarder",
            OS: "Votre système d'exploitation",
            restartDownload1: "Votre téléchargement commencera dans un instant.",
            restartDownload2: "Si ce n'est pas le cas,",
            restartDownload3: "recommencez le téléchargement",
            macMessage: "Après avoir exécuté <strong class='text-primary'>[Terminal]</strong>, copiez et collez le texte ci-dessous.",
        },
        Community: {
            title: "Communauté Evil Spartan",
            description: "Chaînes Twitch, Builds et Joueurs mis en avant",
        }

    },
    pt: {}
}
